...
### 5. HTTP 消息发送、幂等、历史消息与软撤回

阅读顺序：

1. [`controllers/message_controller.go`](../controllers/message_controller.go)：`Create`、`Index`、`MarkRead`、`Recall` 四个入口。
2. [`models/message_model.go`](../models/message_model.go)：重点阅读 `SendMessageWithResult` 和 `RecallMessage`。
...
`RecallMessage` 是软撤回：只把发送者本人消息的 `status` 标记为 `recalled`，保留数据库中的原始 `content`。历史消息、搜索、未读统计和会话最后消息都只基于 `normal` 消息；撤回成功后 Controller 会发布 `conversation.changed`，`change_type = message`，让在线客户端用 HTTP 快照修正本地展示。
