...
`flow-talk-server` 是“流言”的后端服务，当前定位是一个可以本地独立运行、并可选使用 Redis 扩展到多实例部署的即时通讯服务。服务基于 Gin + GORM + MySQL，实现用户注册登录、会话、消息、消息软撤回、资源上传、WebSocket 实时投递、在线状态和消息回执等基础能力；Redis 默认关闭，启用后负责跨实例事件广播和全局在线状态。
...
当前实现包含消息软撤回，但不包含消息硬删除、消息编辑、`delivered` 回执、离线推送和 refresh token。消息状态包含 `normal` 和 `recalled`；`system` 消息类型已预留，但普通客户端不能发送。

### 配套 Web 客户端

同级 `flow-talk-web` 项目已经按上述能力完成主要业务闭环：

- 支持注册、账号密码登录和 `demo` provider 外部身份登录
- 联系人列表可直接创建或进入单聊，也可创建群聊并选择初始成员
- 展示会话摘要、未读数、在线状态和 WebSocket 连接状态
- WebSocket 优先发送文本或图片消息；断线或 ACK 超时后使用相同 `client_msg_id` 通过 HTTP 幂等重试
- 支持失败消息手动重试、游标分页加载历史消息、会话内搜索和全局搜索
- 打开会话或实时收到新消息时同步会话级已读游标，并只为越过旧游标的新消息写入单条已读回执
- 群管理界面按 owner、admin、member 权限显示资料编辑、成员添加、角色调整、成员移除和退出操作
- 首次加载、WebSocket 重连和页面重新可见时通过 HTTP 快照校准会话、成员和在线状态；并发校准请求会复用同一个进行中的任务
- 支持设备上报、设备列表和删除；响应式窄屏通过抽屉访问会话详情

前端只保留服务端实际存在的接口封装；已接入消息软撤回，没有消息硬删除、编辑或 `delivered` 上报等占位调用。
...
会话列表返回会话摘要、单聊 `direct_key`、最后消息、未读数和未读版本，但不返回成员列表；完整成员及其状态通过会话详情接口返回。前端可以使用 `direct_key` 识别单聊对方，再通过用户列表补齐昵称和头像。

会话列表的基础信息、最后消息和未读计数使用固定次数的批量查询完成，不会因会话数量增加而产生逐会话查询的 N+1 问题。
...
更新会话摘要时只允许更大的消息 ID 推进 `last_message_id`，避免并发发送事务乱序提交后把摘要回退到旧消息。
...
消息撤回是软撤回：服务端只把发送者本人消息的 `messages.status` 从 `normal` 更新为 `recalled`，不会删除原始 `content`。历史消息、搜索、未读统计和会话最后消息都只基于 `status = normal`；如果撤回的是会话最后一条消息，服务端会把会话摘要回退到上一条正常消息，没有上一条则置空。撤回成功后服务端发布 `conversation.changed`，`change_type = message`，客户端用会话和历史消息快照校准本地 UI。
...
资源上传使用 `multipart/form-data`，字段为 `file` 和 `type`。`type` 只支持 `image`、`video`，请求体上限为 100 MiB；文件按当前用户和纳秒时间戳保存到 `static/images` 或 `static/videos`。服务端同时校验扩展名白名单和常见格式文件头；当前仍不做病毒扫描或内容审核。上传后的 `/api/static/*` 地址是公开静态资源，不要求 JWT。
...
- 按当前项目交付约束，前后端业务源码均不保留自动化测试文件；`go test ./...` 在服务端用于全包编译检查，前端使用类型检查、ESLint 和生产构建校验。关键权限、事务、幂等和多实例行为仍应在独立验收环境执行端到端验证。
...
- 消息支持发送者本人软撤回，但尚不支持硬删除、编辑或真正的视频类型；数据库中的消息状态包含 `normal` 和 `recalled`。
...
- 登录限流、弱密码策略、刷新 token、登出和 token 黑名单
